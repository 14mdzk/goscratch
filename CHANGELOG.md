...
- `geo.Point` pgx round-trip — `Point.EncodeEWKB()` / `Point.DecodeEWKB()` encode and decode SRID-4326 EWKB (little-endian, 25 bytes); `Point.Value()` implements `driver.Valuer` for use as a pgx query argument against a `geography(Point,4326)` column; `Point.Scan(src any)` implements `sql.Scanner` and handles both binary (raw EWKB `[]byte`) and text (hex-encoded EWKB string) protocol forms. `EncodeEWKB` rejects NaN/±Inf with `ErrNaNOrInf`. `DecodeEWKB` accepts little- or big-endian WKB with or without SRID flag; rejects non-Point geometry types and SRIDs other than 4326 or 0 with `ErrInvalidWKB`. No new imports beyond the standard library. Verified by a build-tagged integration test (`//go:build integration`) that round-trips eight coordinate pairs through a live `postgis/postgis:18-master` testcontainer. Closes v1.3 roadmap row C2.
