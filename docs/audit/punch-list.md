...
| 3b | [Authz cache infra](./pr-03b-authz-cache-infra.md) — `Authorizer` interface + lifecycle, pluggable `persist.Watcher` (noop/memory/redis), backstop reload tick, incremental policy load, raw-SQL lint guard | Cross-cutting (auth lifecycle + perf) | medium | 6h | in review [#22](https://github.com/14mdzk/goscratch/pull/22) |
